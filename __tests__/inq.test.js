...
describe('chooseWhoToSpeakTo function', () => {
  test('speak to friend', async () => {
      inquirer.prompt = jest.fn().mockResolvedValue({person:'Speak to your friend'});

      await expect(chooseWhoToSpeakTo()).resolves.toEqual("your friend");
  });
  test('speak to old woman', async () => {    
      inquirer.prompt = jest.fn().mockResolvedValue({person:'Speak to the old woman'});
...
describe('chooseClothes function', () => {
  ...
  test('Swimming Trunks', async () => {    
      inquirer.prompt = jest.fn().mockResolvedValue({outfit:'Swimming Trunks'});
...
describe('womanResponse function', () => {
  test('I don\'t understand what\'s happening', async () => {
      inquirer.prompt = jest.fn().mockResolvedValue({response:'I don\'t understand what\'s happening'});

      await expect(womanResponse()).resolves.toEqual('I don\'t understand what\'s happening');
  });
  test('Not yet', async () => {
...
describe('chooseASong function', () => {
  test('Beethoven: Moonlight Sonata', async () => {
      inquirer.prompt = jest.fn().mockResolvedValue({song:'Beethoven: Moonlight Sonata'});

      await expect(chooseASong()).resolves.toEqual(0);
  });
  test('Elton John: Rocketman', async () => {
    inquirer.prompt = jest.fn().mockResolvedValue({song:'Elton John: Rocketman'});

    await expect(chooseASong()).resolves.toEqual(1);
  });
  test('Don\'t play anything, just cry', async () => {
    inquirer.prompt = jest.fn().mockResolvedValue({song:'Don\'t play anything, just cry'});
