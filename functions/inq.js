...
const chooseWhoToSpeakTo = async () => {
  ...
  const { person } = await inquirer.prompt({
    ...
    choices,
    name: 'person',
    message: 'You notice your friend waking up next to you with an old woman screaming at you both'
  })
...
const chooseClothes = async () => {
  ...
  const { outfit } = await inquirer.prompt({
    ...
    choices,
    name: 'outfit',
    message: 'She leads you into the dressing room and leaves you alone to get changed. In the wardrobe you see 3 options:'
  });
...
const chooseASong = async () => {
  ...
  const { song } = await inquirer.prompt({
    ...
    choices,
    name: 'song',
    message: 'She pushed you on stage. The crowd turn silent and everyone is staring at you as you walk towards the piano. Knees weak, arms are heavy. You finally sit down at the piano. What do you want to play?'
  });
